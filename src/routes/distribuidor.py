...
app_distribuidor = Blueprint('app_distribuidor', __name__, url_prefix='/distribuidor')
...
def registrar_evento(Componente, Descripcion, tipo_evento):
    ...
    formato_fecha_hora_minuto= datetime.now()

    descripcion_evento = f"{Descripcion}."
    identificacion_componente_alarma = Componente
    username = current_user.Username  # Acceder al username
...
@app_distribuidor.route('/operaciones', methods=['POST', 'GET'])
@login_required
def operaciones():
    with VerificarPermisosUsuario("DVentasConsultar", current_user.RolPerms) as contexto:
        ...
        Componente = 'distribuidor/operaciones'#checar o cambiar
        Descripcion = None
        Tipo_Evento = None
        ...
        if  request.method == 'POST':
            ...
            if operacion == 'consultar_registros':
                try:
                    ...
                    consulta, total_consultas = scaizen_db.OrdenesDeOperacionCarga.select_given_between_date_for_pagination(fecha_inicio,fecha_fin,page_num,elementos_por_pagina)
                    
                    Descripcion = f'Obtención y recuperación de registros de ventas distribuidor periodo {fecha_inicio}-{fecha_fin}  '
                    Tipo_Evento = 3
                    registrar_evento(Componente,Descripcion,Tipo_Evento)
                    ...
                    logging.debug(e)
                    traceback()
                    print("Error al enviar la infórmacion del la BDD")
                    Descripcion = f'Error en la obtención y recuperación de registros de ventas distribuidor periodo {fecha_inicio}-{fecha_fin}'
                    Tipo_Evento = 19 #Cambiar
                    registrar_evento(Componente,Descripcion,Tipo_Evento)
                ...
                try:
                    ...
                    if venta_cancelar:
                        Descripcion = f'Cancelación de venta con orden {cancelar_1}'
                        Tipo_Evento = 3
                        registrar_evento(Componente,Descripcion,Tipo_Evento)
                        return jsonify({'estatus': True, 'status': 'success'}), 200
                    else:
                        Descripcion = f'Intento de cancelación de venta con orden {cancelar_1}'
                        Tipo_Evento = 19
                        registrar_evento(Componente,Descripcion,Tipo_Evento)

                        return jsonify({'estatus': False, 'status': 'error'}), 500
                except Exception as e:
                    Descripcion= f'Error al cancelar la venta con orden {cancelar_1}'
                    Tipo_Evento = 19
                    registrar_evento(Componente, Descripcion, Tipo_Evento)

                    return jsonify({'estatus': False, 'status': 'error', 'message': str(e)}), 500
        return render_template('scaizen/distribuidor_operaciones.html', endpoint='consultar_ventas', productos_color = config.PRODUCTOS_COLOR_BY_NAME, PRODUCTOS_NAMES = config.PRODUCTOS_NAMES)

@app_distribuidor.route('/subir_factura_complemento/<string:prefijo>/<string:orden_id>', methods=['GET', 'POST'])
@login_required
def subir_factura_complemento(prefijo, orden_id):
    componente = 'distribuidor/subir_factura'
    Descripcion = None
    Tipo_Evento = None
    if request.method == 'POST':
        if prefijo == "Venta":
            with VerificarPermisosUsuario("DVentasSubirFacturaXML", current_user.RolPerms) as contexto:
                if contexto is False:  # No tiene permisos
                    return jsonify({'error':'El usuario actual no tiene los permisos para ejecutar esta accion.','action':'redirect'})
        else:
            with VerificarPermisosUsuario("DComprasSubirFacturaXML", current_user.RolPerms) as contexto:
                if contexto is False:  # No tiene permisos
                    return jsonify({'error':'El usuario actual no tiene los permisos para ejecutar esta accion.','action':'redirect'})
        try:
            if 'IdComplemento' in request.form:
                IdComplemento = request.form['IdComplemento']
                datacomplemento = scaizen_db.OrdenesDeOperacionCarga.get_by_id_orden_fk(IdComplemento);
                Data = {}

                if datacomplemento:
                    datacomplemento.producto = config.GET_PRODUCTOS_PRODUCT_NAME(datacomplemento.producto)
                    cliente_info = scaizen_db.Cliente.select_cliente_by_id(datacomplemento.id_destinatario_cliente_fk)
                    estacion_info = scaizen_db.EstacionesDeServicio.select_by_id(datacomplemento.id_destinatario_estacion_de_servicio_fk)
                    autotanque = scaizen_db.Autotanque.select_autotanque_by_id(datacomplemento.id_autotanque_fk)
                    tanque = scaizen_db.Tanque.get_for_id(datacomplemento.id_tanque_fk)
                    #datacomplemento = cv.query_to_json([datacomplemento])[0]

                    batch = scaizen_db.BatchUclCargaDescarga.get_by_id_orden_fk(datacomplemento.id_orden_fk)
                    
                    Data['Producto'] = config.GET_PRODUCTOS_PRODUCT_NAME(batch.producto) if batch else None
                    Data['VolumenNatural'] = format(batch.volumen_natural,".3f") if batch else None
                    Data['VolumenNeto'] = format(batch.volumen_neto,".3f") if batch else None
                    Data['TemperaturaPromedio'] = format(batch.temperatura_promedio,".1f") if batch else None
                    Data['FechaInicio'] = batch.fecha_inicio.strftime("%d/%m/%Y %H:%M:%S") if batch else None
                    Data['FechaFin'] = batch.fecha_fin.strftime("%d/%m/%Y %H:%M:%S") if batch else None
                    Data['Emisor'] = "DISTRIBUIDOR DE DIESEL DE PUEBLA"
                    Data['EmisorRFC'] = 'DDP860722DN4'
                    Data['Receptor'] = cliente_info.nombre_comercial if cliente_info and not estacion_info else estacion_info.nombre_comercial if estacion_info and not cliente_info else "Desconocido"
                    Data['ReceptorRFC'] = cliente_info.rfc if cliente_info and not estacion_info else estacion_info.rfc if estacion_info and not cliente_info else "Desconocido"
                    Data['Autotanque'] = autotanque.numero if autotanque else "Desconocido"
                    Data['Tanque'] = tanque.codigo if tanque else "Desconocido"
                    ...
                    return jsonify({
                        'success': False,
                        'data': f'La orden de complemento con el numero {IdComplemento} no fue encontrada',
                    }), 400
            ...
            if file.filename.endswith('.xml') and file.mimetype in ['application/xml', 'text/xml']:
                ...
                upload_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "facturas_xml/distribuidor/cargas"))
                ...
                autotanque = AddendaMGCLO.find(f'Vehiculo').get('Valor') if AddendaMGCLO else 'Desconocido'
                volumen = float(Producto_data.get('Cantidad')) if Producto_data is not None else None
                ...
                data_return = False
                try:   
                    CfisDistribuidor_id  =cv.CfdisDistribuidor.add(1, CFDI,TypeCFDI,PrecioVentaOCompraOContrap, FechaYHoraTransaccion,VolumenDocumentado,UM)                
                    
                    logging.debug(prefijo)
                    session = cv.SessionLocal()
                    id = config.desencriptar_desde_hex(clave_hex_hash,orden_id)
                    logging.debug(request.form['complemento_id'])
                    complemento_id = request.form['complemento_id'] if request.form['complemento_id'] != "null"  else None
                    logging.debug(f"complemento_id: {complemento_id}")
                    complemento = scaizen_db.BatchUclCargaDescarga.get_by_id_orden_fk(complemento_id) if complemento_id else None
                    complemento_id = complemento_id if complemento_id  else -1
                    logging.debug(f"complemento_id: {complemento_id}")
                    if prefijo == "Venta":
                        operacion = scaizen_db.OrdenesDeOperacionCarga.get_by_id_orden_fk(id)
                        orden = scaizen_db.BatchUclCargaDescarga.get_by_id_orden_fk(id)
                        notok = []
                        
                        if orden:
                            orden.Id_cliente_cv_fk = -1
                            orden.Id_estacion_cv_fk = -1
                            cliente_info = scaizen_db.Cliente.select_cliente_by_id(operacion.id_destinatario_cliente_fk) if operacion else None
                            estacion_info = scaizen_db.EstacionesDeServicio.select_by_id(operacion.id_destinatario_estacion_de_servicio_fk) if operacion else None
                            autotanque = scaizen_db.Autotanque.select_autotanque_by_id(operacion.id_autotanque_fk) if operacion else None
                            tanque = scaizen_db.Tanque.get_for_id(operacion.id_tanque_fk) if operacion else None
                            if cliente_info:
                                cliente =  cv.ClienteCV.select_by_rfc(cliente_info.rfc)
                                orden.Id_cliente_cv_fk = cliente.Id_cliente if cliente else -1
                            if estacion_info:
                                estacion_servicio = cv.EstacionesCV.select_by_rfc(estacion_info.rfc)
                                orden.Id_estacion_cv_fk = estacion_servicio.Id_estacion if estacion_servicio else -1
                            orden.Autotanque = autotanque.numero if autotanque else "Desconocido"
                            orden.TanqueCodigo = tanque.codigo if tanque else "Desconocido"
                            
                            telemedicon_tanque = scaizen_db.Telemedicion_tanques.get_for_id_tanque_fk(orden.id_tanque_fk)
                            medicion_tanque_start = scaizen_db.Rt_tanques.get_for_id_instrumentacion_fk_start(telemedicon_tanque.id_instrumentacion_fk, orden.fecha_inicio) if telemedicon_tanque else None
                            medicion_tanque_end = scaizen_db.Rt_tanques.get_for_id_instrumentacion_end(telemedicon_tanque.id_instrumentacion_fk,orden.fecha_fin) if telemedicon_tanque else None
                            orden.VolumenInicialTanque = 0 
                            orden.VolumenFinalTanque = 0
                            if medicion_tanque_start:
                                MTanqueJsonStart = medicion_tanque_start.json
                                orden.VolumenInicialTanque = MTanqueJsonStart['ucl_telemedicion']['data']['volumen_natural']
                            if medicion_tanque_end:
                                MTanqueJsonEnd = medicion_tanque_end.json
                                orden.VolumenFinalTanque =MTanqueJsonEnd['ucl_telemedicion']['data']['volumen_natural']
                        
                        if complemento:
                            operacion = scaizen_db.OrdenesDeOperacionCarga.get_by_id_orden_fk(complemento_id)
                            cliente_info = scaizen_db.Cliente.select_cliente_by_id(operacion.id_destinatario_cliente_fk) if operacion else None
                            estacion_info = scaizen_db.EstacionesDeServicio.select_by_id(operacion.id_destinatario_estacion_de_servicio_fk) if operacion else None
                            autotanque = scaizen_db.Autotanque.select_autotanque_by_id(operacion.id_autotanque_fk) if operacion else None
                            tanque = scaizen_db.Tanque.get_for_id(operacion.id_tanque_fk) if operacion else None
                            if cliente_info:
                                cliente =  cv.ClienteCV.select_by_rfc(cliente_info.rfc)
                                complemento.Id_cliente_cv_fk = cliente.Id_cliente if cliente else -1
                            if estacion_info:
                                estacion_servicio = cv.EstacionesCV.select_by_rfc(estacion_info.rfc)
                                complemento.Id_estacion_cv_fk = estacion_servicio.Id_estacion if estacion_servicio else -1
                            complemento.Autotanque = autotanque.numero if autotanque else "Desconocido"
                            complemento.TanqueCodigo = tanque.codigo if tanque else "Desconocido"


                            
                            if orden.TanqueCodigo != complemento.TanqueCodigo:
                                notok.append(f"Los Tanques no coinciden({orden.TanqueCodigo} y {complemento.TanqueCodigo})")
                            if orden.producto != complemento.producto:
                                notok.append(f"Los Productos no coinciden({orden.producto} y {complemento.producto})")
                            if orden.Autotanque != complemento.Autotanque:
                                notok.append(f"Los Autotanque no coinciden({orden.Autotanque} y {complemento.Autotanque})")

                            if len(notok) > 0:
                                notok_values = "\n".join(str(value) for value in notok)  # Recorre notok y convierte cada valor a string
                                logging.debug(notok_values)
                                return jsonify({
                                    'success': False,
                                    'message': f'La operacion no puede ser llevada acabo por que los siguientes valores no coinciden:\n{notok_values}',
                                }), 400
                                

                            logging.debug(f"ORDEN: {orden}")

                            telemedicon_tanque = scaizen_db.Telemedicion_tanques.get_for_id_tanque_fk(complemento.id_tanque_fk)
                            medicion_tanque_start = scaizen_db.Rt_tanques.get_for_id_instrumentacion_fk_start(telemedicon_tanque.id_instrumentacion_fk, complemento.fecha_inicio) if telemedicon_tanque else None
                            medicion_tanque_end = scaizen_db.Rt_tanques.get_for_id_instrumentacion_end(telemedicon_tanque.id_instrumentacion_fk,complemento.fecha_fin) if telemedicon_tanque else None
                            complemento.VolumenInicialTanque = 0 
                            complemento.VolumenFinalTanque = 0
                            if medicion_tanque_start:
                                MTanqueJsonStart = medicion_tanque_start.json
                                complemento.VolumenInicialTanque = MTanqueJsonStart['ucl_telemedicion']['data']['volumen_natural']
                            if medicion_tanque_end:
                                MTanqueJsonEnd = medicion_tanque_end.json
                                complemento.VolumenFinalTanque =MTanqueJsonEnd['ucl_telemedicion']['data']['volumen_natural']

                            #Suma de datos del complemento y la orden
                            orden.volumen_natural = orden.volumen_natural + complemento.volumen_natural
                            orden.volumen_neto = orden.volumen_neto + complemento.volumen_neto
                            orden.temperatura_promedio = (orden.temperatura_promedio + complemento.temperatura_promedio)/2
                            
                            if orden.fecha_inicio > complemento.fecha_inicio: 
                                orden.fecha_inicio = complemento.fecha_inicio
                                orden.VolumenInicialTanque = complemento.VolumenInicialTanque
                            if orden.fecha_fin > complemento.fecha_fin: 
                                orden.fecha_fin = complemento.fecha_fin
                                orden.VolumenFinalTanque = complemento.VolumenFinalTanque
                            #hora_incio_operacion = complemento.fecha_inicio if orden.fecha_inicio > complemento.fecha_inicio else orden.fecha_inicio
                            #hora_fin_operacion =  complemento.fecha_fin if orden.fecha_fin > complemento.fecha_fin else orden.fecha_fin
                        ...
                        data_return = cv.CargaDistribuidor.add(orden.id,orden.id_orden_fk,orden.Autotanque,orden.producto,orden.TanqueCodigo,
                                                "C",orden.VolumenInicialTanque,UMA,orden.VolumenFinalTanque,orden.volumen_natural,UMA,orden.volumen_neto,UMA,
                                                orden.temperatura_promedio,presion,Costo_producto,Costo_traslado,TotalImpuestos,Costo_total,orden.fecha_inicio,orden.fecha_fin,
                                                orden.Id_cliente_cv_fk,orden.Id_estacion_cv_fk,CfisDistribuidor_id,Path(filename).stem,complemento_id)
                        
                        logging.debug("Data Guardada: {data_return}")
                    else:
                        consulta = session.query(cv.DescargasDistribuidor).filter_by(Id=id).first()
                        logging.debug(f" ")
                        logging.debug(f"Consulta compra {consulta}")
                        logging.debug(f" ")
                        if consulta:
                            logging.debug(f"cfdi {CfisDistribuidor_id}")
                            logging.debug(f"orden id {consulta.Id}")
                            consulta.Id_cfdi_distribuidor_fk = CfisDistribuidor_id
                            logging.debug(f"consulta.Id_cfdi_distribuidor_fk {consulta.Id_cfdi_distribuidor_fk}")
                            ...
                            logging.debug("No se encontró la consulta con el Id especificado.")

                except Exception as e:
                    session.rollback()
                    traceback.print_exc()  # Esto imprimirá el traceback del error
                    logging.debug(f"Error al actualizar: {e}")
                    # Registrar el evento de consulta
                    descripcion_evento = f"Error al registrar la operación de carga: {e}"
                    ...
                    session.close()
                    if data_return:
                        descripcion_evento = f"Se ha registrado la operación de carga con éxito: {data_return.Id}"
                        identificacion_componente_alarma = "Agregar operación"
                        EventosDistribuidor.add(
                            datetime.now(), current_user.Username, 11, descripcion_evento, identificacion_componente_alarma
                        )
                        return jsonify({
                            'success': True,
                            'message': f'La orden ha sido registrada con éxito y se enlazo a la factura: {filename}',
                        }), 200
                    else:
                        return jsonify({
                            'success': False,
                            'message': f'La orden no pudo ser registrada con éxito y se enlazo a la factura: {filename}',
                        }), 200
            else:
        ...
        if prefijo == "Venta":
            with VerificarPermisosUsuario("DVentasSubirFacturaXML", current_user.RolPerms) as contexto:
                ...
                if operacion:
                    ...
                    cliente_info = scaizen_db.Cliente.select_cliente_by_id(operacion.id_destinatario_cliente_fk)
                    estacion_info = scaizen_db.EstacionesDeServicio.select_by_id(operacion.id_destinatario_estacion_de_servicio_fk)
                    autotanque = scaizen_db.Autotanque.select_autotanque_by_id(operacion.id_autotanque_fk)
                    tanque = scaizen_db.Tanque.get_for_id(operacion.id_tanque_fk)
                    #operacion = cv.query_to_json([operacion])[0]

                    batch = scaizen_db.BatchUclCargaDescarga.get_by_id_orden_fk(operacion.id_orden_fk)
                    Data['Producto'] =  config.GET_PRODUCTOS_PRODUCT_NAME(batch.producto) if batch else None
                    Data['VolumenNatural'] = batch.volumen_natural if batch else None
                    Data['VolumenNeto'] = batch.volumen_neto if batch else None
                    Data['TemperaturaPromedio'] = format(batch.temperatura_promedio,".1f") if batch else None
                    Data['FechaInicio'] = batch.fecha_inicio.strftime("%d/%m/%Y %H:%M:%S") if batch else None
                    Data['FechaFin'] = batch.fecha_fin.strftime("%d/%m/%Y %H:%M:%S") if batch else None
                    Data['Emisor'] = "DISTRIBUIDOR DE DIESEL DE PUEBLA"
            ...
            with VerificarPermisosUsuario("DComprasSubirFacturaXML", current_user.RolPerms) as contexto:
                if contexto is False:  # No tiene permisos
                    return redirect("/scaizen/")
            operacion = cv.DescargasDistribuidor.select_by_id(id)
            
            if operacion:
                if operacion.Id_cfdi_distribuidor_fk != None:
                    return redirect("/scaizen/")
                operacion.Producto = config.GET_PRODUCTOS_PRODUCT_NAME(operacion.Producto)
        ...
        return render_template('scaizen/subir_factura_complemento.html', operacion=Data, PRODUCTOS_NAMES = config.PRODUCTOS_NAMES,CLAVES_PRODUCTOS = config.CLAVES_PRODUCTOS,
                            CLAVES_SERVICIOS = config.CLAVES_SERVICIOS, blueprint='app_distribuidor', prefijo=prefijo )

@app_distribuidor.route('/consultar_ventas', methods=['POST', 'GET'])
@login_required
        ...
        if  request.method == 'POST':
            ...
            if operacion == 'consultar_registros':
                try:
                    ...
                    logging.debug(f"fecha_inicio: {fecha_inicio}")
                    logging.debug(f"fecha_fin: {fecha_fin}")
                    consulta, total_consultas = cv.CargaDistribuidor.select_given_between_date_for_pagination(fecha_inicio,fecha_fin,page_num,elementos_por_pagina)
                    ...
                    print("Error al enviar la infórmacion del la BDD")
                    Descripcion = f'Erro en la obtención y recuperación de registros de ventas distribuidor periodo {fecha_inicio}-{fecha_fin}'
                    Tipo_Evento = 19 #Cambiar
                    registrar_evento(Componente,Descripcion,Tipo_Evento)
                ...
                if parametro == "presion":
                    ...
                    if value == "": message = f"Valor vació"
                        ...
                        try:
                            carga = session.query(cv.CargaDistribuidor).filter(cv.CargaDistribuidor.Id == order_id).first()
                            if carga:
                ...
                try:
                    ...
                    return jsonify({'estatus': False, 'status': 'error', 'message': str(e)}), 500
        return render_template('scaizen/distribuidor_consultar_operaciones.html', endpoint='consultar_ventas', productos_color = config.PRODUCTOS_COLOR_BY_NAME, PRODUCTOS_NAMES = config.PRODUCTOS_NAMES)


@app_distribuidor.route('/subir_factura/<string:prefijo>/<string:orden_id>', methods=['GET', 'POST'])
@login_required
def subir_factura(prefijo, orden_id):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if 'IdComplemento' in request.form:
                IdComplemento = request.form['IdComplemento']
                datacomplemento = cv.CargaDistribuidor.select_by_id(IdComplemento)

                if datacomplemento:
                    datacomplemento.Producto = config.GET_PRODUCTOS_PRODUCT_NAME(datacomplemento.Producto)
                    cliente = cv.ClienteCV.select_by_Id_cliente(datacomplemento.Id_cliente_cv_fk)
                    estacion = cv.EstacionesCV.select_by_Id_estacion(datacomplemento.Id_estacion_cv_fk)
                    datacomplemento.id_entregas_fk = None
                    datacomplemento.Id_cfdi_distribuidor_fk = None
                    datacomplemento.Id = None
                    datacomplemento.Id_fk = None
                    datacomplemento.Id_cliente_cv_fk = None
                    datacomplemento.Id_estacion_cv_fk = None
                    datacomplemento.FechaYHoraInicialEntrega = datacomplemento.FechaYHoraInicialEntrega.strftime("%d/%m/%Y %H:%M")
                    datacomplemento.FechaYHoraFinalEntrega = datacomplemento.FechaYHoraFinalEntrega.strftime("%d/%m/%Y %H:%M")
                    datacomplemento = cv.query_to_json([datacomplemento])[0]
                    datacomplemento['Emisor'] = "DISTRIBUIDOR DE DIESEL DE PUEBLA"
                    datacomplemento['EmisorRFC'] = 'DDP860722DN4'
                    datacomplemento['Receptor'] = cliente.Nombre_comercial if cliente and not estacion else estacion.Nombre_comercial if estacion and not cliente else "Desconocido"
                    datacomplemento['ReceptorRFC'] = cliente.RFC if cliente and not estacion else estacion.RFC if estacion and not cliente else "Desconocido"
                    
                    datacomplemento = cv.query_to_json([datacomplemento])[0]
                    logging.debug(datacomplemento)
                    return jsonify({
                        'success': True,
            ...
            if file.filename.endswith('.xml') and file.mimetype in ['application/xml', 'text/xml']:
                # Guardar el archivo en un directorio seguro
                upload_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "facturas_xml/distribuidor/descargas"))
                logging.debug(upload_dir)
                ...
                try:   
                    ...
                    id = config.desencriptar_desde_hex(clave_hex_hash,orden_id)
                    if prefijo == "Venta":
                        consulta = session.query(cv.CargaDistribuidor).filter_by(Id=id).first()
        ...
        if prefijo == "Venta":
            with VerificarPermisosUsuario("DVentasSubirFacturaXML", current_user.RolPerms) as contexto:
                if contexto is False:  # No tiene permisos
                    return redirect("/scaizen/")
            operacion = cv.CargaDistribuidor.select_by_id(id)
            if operacion:
                if operacion.Id_cfdi_distribuidor_fk != None:
                    return redirect("/scaizen/")
                operacion.Producto = config.GET_PRODUCTOS_PRODUCT_NAME(operacion.Producto)
                ...
                operacion.Id_cfdi_distribuidor_fk = None
                operacion.Id = None
                operacion.Id_fk = None
...
@app_distribuidor.route('/compra_alta', methods=['GET', 'POST'])
@login_required
def compra_alta():
    with VerificarPermisosUsuario("DComprasAlta", current_user.RolPerms) as contexto:
        if contexto is False:  # No tiene permisos
            return redirect("/scaizen/")
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            file = request.files['file']
            dates_tart = request.form['dates_tart']
            ...
            if file.filename.endswith('.xml') and file.mimetype in ['application/xml', 'text/xml']:
                ...
                try:
                    CfdisDistribuidor_id = cv.CfdisDistribuidor.add(1, CFDI, TypeCFDI, PrecioVentaOCompraOContrap, FechaYHoraTransaccion, VolumenDocumentado, UM)
                    print(f"cfdi {CfdisDistribuidor_id}")
                    session = cv.SessionLocal()

                    volumen_inicial_tanque = 0
                    volumen_final_tanque = 0
                    tanque_element = scaizen_db.Tanque.get_for_codigo(tanque)
                    if tanque_element:
                        telemedicon_tanque = scaizen_db.Telemedicion_tanques.get_for_id_tanque_fk(tanque_element.id)
                        logging.debug(f'tanque_element: {tanque_element}')
                        logging.debug(f'tanque_element.id: {tanque_element.id}')
                        logging.debug(f'telemedicon_tanque: {telemedicon_tanque}')
                        if telemedicon_tanque:
                            medicion_tanque_start = scaizen_db.Rt_tanques.get_for_id_instrumentacion_fk_start(telemedicon_tanque.id_instrumentacion_fk, dates_tart)
                            medicion_tanque_end = scaizen_db.Rt_tanques.get_for_id_instrumentacion_fk_start(telemedicon_tanque.id_instrumentacion_fk, dates_tart)
                            logging.debug(f"medicion_tanque_start: {medicion_tanque_start}")
                            logging.debug(f"medicion_tanque_end: {medicion_tanque_end}")
                            if medicion_tanque_start:
                                MTanqueJsonStart = medicion_tanque_start.json
                                volumen_inicial_tanque = MTanqueJsonStart['ucl_telemedicion']['data']['volumen_natural']
                            if medicion_tanque_end:
                                MTanqueJsonEnd = medicion_tanque_end.json
                                volumen_final_tanque = MTanqueJsonEnd['ucl_telemedicion']['data']['volumen_natural']
                    logging.debug("Antes de registrar");

                    return_data = cv.DescargasDistribuidor.add(CfdisDistribuidor_id,provedor_id,autotanque,producto,tanque,'D',volumen_inicial_tanque,UM,volumen_final_tanque,
                                                cantidad_nat_recibida,UM,cantidad_neto_recibida,UM,temperatura,1.033,Costo_producto,Costo_traslado,TotalImpuestos,Costo_total,dates_tart,date_end,Path(filename).stem,1)
                    logging.debug(return_data)
                    logging.debug("Guardando Archivo")
                    file.save(filepath)
        ...
        tanques = []
        tanques_model = cv.Tanque.select_all()
        if tanques_model:
            tanques = [model_tanque.codigo for model_tanque in tanques_model]

    # Si la solicitud no es POST, también se debe manejar el caso GET (podrías querer devolver algo en este caso)
        return render_template('scaizen/distribuidor_alta_compra.html', tanques=tanques)

@app_distribuidor.route('/consultar_compras', methods=['POST', 'GET'])
@login_required
def consultar_compras():
    with VerificarPermisosUsuario("DComprasConsultar", current_user.RolPerms) as contexto:
        if contexto is False:  # No tiene permisos
            return redirect("/scaizen/")
        componente = 'distribuidor/consultar_compras'
        Descripcion = None
        Tipo_Evento = None
        ...
        if  request.method == 'POST':
            ...
            if operacion == 'consultar_registros':
                ...
                logging.debug(f"fecha_fin: {fecha_fin}")
                elementos_por_pagina = 10
                consulta, total_consultas = cv.DescargasDistribuidor.select_given_between_date_for_pagination(fecha_inicio,fecha_fin,page_num,elementos_por_pagina)
                ...
                if parametro == "presion":
                    ...
                    if value == "": message = f"Valor vació"
                        ...
                        try:
                            descarga = session.query(cv.DescargasDistribuidor).filter(cv.DescargasDistribuidor.Id == order_id).first()
                            if descarga:
        ...
        return render_template('scaizen/distribuidor_consultar_operaciones.html',endpoint='consultar_compras', productos_color = config.PRODUCTOS_COLOR_BY_NAME, PRODUCTOS_NAMES = config.PRODUCTOS_NAMES)
...
@app_distribuidor.route('/diario_tanques', methods=['GET', 'POST'])
@login_required
def diario_tanques():
    with VerificarPermisosUsuario("DReportediario", current_user.RolPerms) as contexto:
        if contexto is False:  # No tiene permisos
            return redirect("/scaizen/")
    
    componente = 'distribuidor/diario_tanques'
    Descripcion = None
    Tipo_Evento = None


    GeneralReportInstance = scaizen_cv_funtions.GeneralReport(1)
    productos = GeneralReportInstance.GiveProductsIdsAndMarcaC()
    logging.debug('productos')
    logging.debug(productos)
    productos_options = []
    ...
    if request.method == 'POST':
        ...
        DailyReportFuntionsInstance= scaizen_cv_funtions.DailyReportFuntions(1)

        if generar:
            with VerificarPermisosUsuario("DReportediarioGenerar", current_user.RolPerms) as contexto:
                if contexto is False:  # No tiene permisos
                    return jsonify({'error':'El usuario actual no tiene los permisos para ejecutar esta accion.','action':'redirect'})
            ...
            with VerificarPermisosUsuario("DReportediarioVer", current_user.RolPerms) as contexto:
                if contexto is False:  # No tiene permisos
                    return jsonify({'error':'El usuario actual no tiene los permisos para ejecutar esta accion.','action':'redirect'})
            try:
                ...
                if DailyRepor:
                    for d in DailyRepor:
                        for t in d['tanques']:
                            for r in t['recepcion']:
                                logging.debug(cv.query_to_json([r]))
                                logging.debug("")
                    logging.debug('DailyRepor')
                    logging.debug(DailyRepor)
    ...
    return render_template('scaizen/informe_tanques.html', productos_options=productos_options,blueprint='app_distribuidor',
            ...
            for producto in diarios_JSON:
                for tanque in producto['tanques']:
                    for recepcion in tanque['recepcion']:
                        ...
                        complemento_distribuidor_recepcion = Comp_Comercializador.Comp_Comercializacion_schema_json(id_complemento_recepcion,tipo,'D',recepcion)
                    for entrega in tanque['entrega']:
                        id_complemento_entrega = 2
                        complemento_distribuidor_entrega = Comp_Comercializador.Comp_Comercializacion_schema_json(id_complemento_entrega,tipo,'C',entrega)
                    ...
                    tanque_item={
                        ...
                        'Recepciones': {
                            ...
                            'Recepcion': [{
                                ...
                                'FechaYHoraInicioRecepcion':recepcion.FechaYHoraInicialRecepcion.astimezone().isoformat(),
                                'FechaYHoraFinalRecepcion':recepcion.FechaYHoraFinalRecepcion.astimezone().isoformat(), 
                                'Complemento':Comp_Comercializador.Comp_Comercializacion_schema_json(id_complemento_recepcion,tipo,'D',recepcion)
                        ...
                        'Entregas': {
                            ...
                            'Entrega': [{
                                ...
                                'FechaYHoraInicialEntrega':entrega.FechaYHoraInicialEntrega.astimezone().isoformat(),
                                'FechaYHoraFinalEntrega': entrega.FechaYHoraFinalEntrega.astimezone().isoformat(),
                                'Complemento':Comp_Comercializador.Comp_Comercializacion_schema_json(id_complemento_entrega,tipo,'C',entrega)
...
@app_distribuidor.route('/informe_de_tanque_JSON', methods=['GET','POST'])
@login_required
def informe_de_tanque_JSON():
    ...
    componente = 'distribuidor/informe_de_tanque_JSON'
    Descripcion = None
    Tipo_Evento = None
                ...
                for producto in diarios_JSON:
                    for tanque in producto['tanques']:
                        for recepcion in tanque['recepcion']:
                            ...
                            complemento_distribuidor_recepcion = Comp_Distribucion.Comp_Distribucion_schema_json(id_complemento_recepcion,tipo,'D',recepcion)
                        for entrega in tanque['entrega']:
                            id_complemento_entrega = 1
                            print(f"aqui  {id_complemento_entrega}")
                            complemento_distribuidor_entrega = Comp_Distribucion.Comp_Distribucion_schema_json(id_complemento_entrega,tipo,'C',entrega)
                print("hola2")
...
@app_distribuidor.route('/informe_de_tanque_XML', methods=['GET','POST'])
@login_required
def informe_de_tanque_XML():
    ...
    componente = 'distribuidor/informe_de_tanque_XML'
    Descripcion = None
    Tipo_Evento = None
    ...
    if fecha_consulta:
        try:
            ...
            if diarios_JSON:
                ...
                schema_file = os.path.abspath(os.path.join(os.path.dirname(__file__), "..","static/schemes/distribuidor/Diario.xsd"))
                ...
                for producto_ in diarios_JSON:
                    ...
                    for tanques in producto_['tanques']:
                        ...
                        for recepcion in tanques['recepcion']:
                            ...
                            id_complemento_recepcion = 1
                            DailyReportFuntionsInstance = DailyReportFuntions(1)
                            complementos_date = DailyReportFuntionsInstance.GetComplementDailyReportAll(id_complemento_recepcion,tipo,'D',recepcion)
...
def generate_daily_pdf(DailyRepor,periodo):
    ...
    General_Report = scaizen_cv_funtions.GeneralReport(1)
    ...
    for i, producto in enumerate(DailyRepor):
        ...
        for tanque in producto['tanques']:
            ...
            rows = []
            sizes = [12, 9, 9, 4, 9, 9, 4, 7, 7, 12, 9, 9] #Porcentaje
            for recepcion in tanque['recepcion']:
                row = [recepcion.FechaYHoraInicialRecepcion.strftime("%Y-%m-%d %H:%M:%S"),recepcion.VolumenInicialTanque,'',recepcion.Id,recepcion.VolumenRecepcion,recepcion.VolumenRecepcionNeto,
                    round(recepcion.Temperatura, 2),round(recepcion.PresionAbsoluta, 2),'',recepcion.FechaYHoraFinalRecepcion.strftime("%Y-%m-%d %H:%M:%S"),recepcion.VolumenFinalTanque,'']
                rows.append(row)
            if not rows:
                rows = [['---' for _ in range(12)]]
            headers_sizes = [30,40,30] #Porcentaje
            headers = ['Volumen Inicial de Tanque','Volumen Recepción / Descargas (Recibido)','Volumen Final de Tanque']
            ...
            for entrega in tanque['entrega']:
                row = [entrega.FechaYHoraInicialEntrega.strftime("%Y-%m-%d %H:%M:%S"),entrega.VolumenInicialTanque,'',entrega.Id,entrega.VolumenEntregado,entrega.VolumenEntregadoNeto,
                    round(entrega.Temperatura, 2),round(entrega.PresionAbsoluta, 2),'',entrega.FechaYHoraFinalEntrega.strftime("%Y-%m-%d %H:%M:%S"),entrega.VolumenFinalTanque,'']
                rows.append(row)
            if not rows:
                rows = [['---' for _ in range(12)]]
            headers_sizes = [30,40,30]
            ...
            sizes2 = [8.5,8.5,8.5,8.5,8.25,8.25,8.25,8.25,8.25,8.25,8.25,8.25] #Porcentaje
            headers = ['Volumen Anterior','Volumen en Recepciones','Volumen en Entregas','Volumen Existente','Total','Documentos',
                    'Volumen Recibido','Compras','Total','Documentos','Volumen Entregado','Ventas']
            ...
            headers_sizes = [34,33,33] #Porcentaje
...
@app_distribuidor.route('/informe_de_tanque_PDF', methods=['GET','POST'])
@login_required
def informe_de_tanque_PDF():
    with VerificarPermisosUsuario("DReportediarioPDF", current_user.RolPerms) as contexto:
        if contexto is False:  # No tiene permisos
            return jsonify({'error':'El usuario actual no tiene los permisos para ejecutar esta accion.','action':'redirect'})
        
    componente = 'distribuidor/informe_de_tanque_PDF'
    Descripcion = None
    Tipo_Evento = None
...
def REPORTE_MENSUAL_COMBINADO_JSON(productos_data,fecha_corte):
    ...
    for producto in productos_data[0]:
        ...
        for carga_c in cargas_c:
            complemento = Comp_Comercializador.Comp_Comercializacion_schema_json(id_complemento_recepcion,tipo,'C',carga_c)
            logging.debug("complemento C")
        ...
        for descarga_c in descargas_c:
            complemento = Comp_Comercializador.Comp_Comercializacion_schema_json(id_complemento_entrega,tipo,'D',descarga_c)
            #logging.debug("complemento D")
            logging.debug(complemento)
            if complemento:
                complemento_distribuidor_recepcion.append(complemento)
    ...
    for producto in productos_data[1]:
        ...
        complemento_distribuidor_entrega = []
        for carga in cargas:
            complemento = Comp_Comercializador.Comp_Comercializacion_schema_json(id_complemento_recepcion,tipo,'C',carga)
            #logging.debug("complemento C")
            logging.debug(complemento)
            if complemento:
                complemento_distribuidor_entrega.append(complemento)
        ...
        print(f"entregas {producto['entregas']}")
        for descarga in descargas:
            complemento = Comp_Comercializador.Comp_Comercializacion_schema_json(id_complemento_entrega,tipo,'D',descarga)
            #logging.debug("complemento D")
            #logging.debug(complemento)
            if complemento:
                complemento_distribuidor_recepcion.append(complemento)
...
def REPORTE_MENSUAL_COMBINADO_XML(productos_data, fecha_corte):
    ...
    componente = 'distribuidor/REPORTE_MENSUAL_XML'
    Descripcion = None
    Tipo_Evento = None
    tipo = str('distribuidor')
    id_complemento_recepcion = 1
    ...
    for producto in productos_data[0]:
        ...
        for carga in cargas:
            #complementos_date = DailyReportFuntionsInstance.GetComplementDailyReportAll(id_complemento_entrega,tipo,'C',entrega)
            #complemento = Comp_Distribucion.Comp_Distribucion_schema_json(id_complemento_recepcion,tipo,'C',carga)
            complemento = DailyReportFuntionsInstance.GetComplementDailyReportAll(id_complemento_entrega,tipo,'C',carga)
            #logging.debug("complemento C")
            #logging.debug(complemento)
            if complemento:
        ...
        for descarga in descargas:
            #complemento = Comp_Distribucion.Comp_Distribucion_schema_json(id_complemento_entrega,tipo,'D',descarga)
            complemento = DailyReportFuntionsInstance.GetComplementDailyReportAll(id_complemento_entrega,tipo,'D',descarga)
            #logging.debug("complemento D")
            #logging.debug(complemento)
            if complemento:
        ...
        if any(complemento_distribuidor_recepcion):
            ...
            complemento_distribuidor_recepcion = DailyReportFuntionsInstance.GetComplementDailyReportAll(id_complemento_entrega,tipo,'D',None)
            for complementos in complemento_distribuidor_recepcion:
                Complemento_Distribucion_Recepcion = ET.SubElement(Complemento, '{%s}Complemento_Distribucion' %covol)
                certificado = complementos['CERTIFICADO']
        ...
        if complemento_distribuidor_entrega:
            ...
            complemento_distribuidor_entrega = DailyReportFuntionsInstance.GetComplementDailyReportAll(id_complemento_entrega,tipo,'C',None)
            for complementos in complemento_distribuidor_entrega:
                Complemento_Distribucion_Entrega = ET.SubElement(Complemento, '{%s}Complemento_Distribucion' %covol)
                certificado = complementos['CERTIFICADO']
    ...
    for producto in productos_data[1]:
        ...
        if complemento_distribuidor_entrega:
            for complementos in complemento_distribuidor_entrega:
                Complemento_Comercializador_Entrega = ET.SubElement(Complemento, '{%s}Complemento_Comercializacion' %covol)
                """
                terminalAlmYTrans = complementos['TERMINALALMYTRANS_TERMINALALMYDIST']
                ...
                Certificado = ET.SubElement(Complemento_Comercializador_Entrega, '{%s}CERTIFICADO' % co)
                ET.SubElement(Certificado, '{%s}RfcCertificado' % co).text = str(certificado['CERTIFICADO'].RfcCertificado)
                ET.SubElement(Certificado, '{%s}NumeroFolioCertificado' % co).text = str(certificado['CERTIFICADO'].NumeroFolioCertificado)
                ET.SubElement(Certificado, '{%s}FechaEmisionCertificado' % co).text = str(certificado['CERTIFICADO'].FechaEmisionCertificado)
                ET.SubElement(Certificado, '{%s}ResultadoCertificado' % co).text = str(certificado['CERTIFICADO'].ResultadoCertificado)   
                ...
                for nacional in complementos['NACIONAL']:
                    Nacional = ET.SubElement(Complemento_Comercializador_Entrega, '{%s}NACIONAL' % co)
                ...
                for extranjero in complementos['EXTRANJERO']:
                    ...
                    for pedimentos in extranjero['pedimentos']:
                                    ...
                                    ET.SubElement(Volumen_documentado, '{%s}UM' % dis).text = str(pedimentos.VolumenDocumentado_UM)
                """                           
                Aclarcion = ET.SubElement(Complemento_Comercializador_Entrega, '{%s}ACLARACION' % co)
                ET.SubElement(Aclarcion, '{%s}Aclaracion' % co).text = str(complementos['ACLARACION']['ACLARACION'])
        else:
            complemento_distribuidor_entrega = DailyReportFuntionsInstance.GetComplementDailyReportAll(id_complemento_entrega,tipo,'C',None)
            for complementos in complemento_distribuidor_entrega:
...
@app_distribuidor.route('/control_mensual_volumetrico', methods=['GET','POST'])
@login_required
def control_mensual_volumetrico():
    with VerificarPermisosUsuario("DReportemensual", current_user.RolPerms) as contexto:
        if contexto is False:  # No tiene permisos
            return redirect("/scaizen/")
    componente = 'distribuidor/control_mensual_volumetrico'
    Descripcion = None
    Tipo_Evento = None
    ...
    if request.method == 'POST':
        with VerificarPermisosUsuario("DReportemensualVer", current_user.RolPerms) as contexto:
            if contexto is False:  # No tiene permisos
                return jsonify({'error':'El usuario actual no tiene los permisos para ejecutar esta accion.','action':'redirect'})
        ...
        elif producto and month and year:
            try:
                if producto != "all":
                    scaizen_funtions = scaizen_cv_funtions.GeneralReport(1)
        ...
        return render_template('scaizen/orden_mes.html', years=years,blueprint='app_distribuidor',productos_color = config.PRODUCTOS_COLOR_BY_NAME, PRODUCTOS_NAMES = config.PRODUCTOS_NAMES)
    # Si la solicitud no es POST, renderizar la plantilla
    return render_template('scaizen/orden_mes.html', years=years,blueprint='app_distribuidor',productos_color = config.PRODUCTOS_COLOR_BY_NAME, PRODUCTOS_NAMES = config.PRODUCTOS_NAMES)

def generate_mensual_pdf(productos,periodo):

    componente = 'distribuidor/generate_mensual_pdf'
    Descripcion = None
    Tipo_Evento = None
...
@app_distribuidor.route('/informe_mensual_pdf', methods=['GET','POST'])
@login_required
def informe_mensual_pdf():
    with VerificarPermisosUsuario("DReportemensualPDF", current_user.RolPerms) as contexto:
        if contexto is False:  # No tiene permisos
            return jsonify({'error':'El usuario actual no tiene los permisos para ejecutar esta accion','action':'redirect'})
            
    componente = 'distribuidor/informe_mensual_pdf'
    Descripcion = None
    Tipo_Evento = None
...
@app_distribuidor.route('/informe_mensual_xml', methods=['GET','POST'])
@login_required
def informe_mensual_xml():
    with VerificarPermisosUsuario("DReportemensualXML", current_user.RolPerms) as contexto:
        if contexto is False:  # No tiene permisos
            return jsonify({'error':'El usuario actual no tiene los permisos para ejecutar esta accion.','action':'redirect'})
    componente = 'distribuidor/informe_mensual_xml'
    Descripcion = None
    Tipo_Evento = None
...
def generate_alarms_events_pdf(data,periodo,type):
    componente = 'distribuidor/generate_alarms_events_pdf'
    Descripcion = None
    Tipo_Evento = None
...
@app_distribuidor.route('/alarms_events_mensual_pdf', methods=['GET','POST'])
@login_required
def alarms_events_mensual_pdf():
    componente = 'distribuidor/alarms_events_mensual_pdf'
    Descripcion = None
    Tipo_Evento = None
    ...
    if datepicker_start and datepicker_end:
        try:
            if type == 'alarmas':
                with VerificarPermisosUsuario("DReportedealarmasPDF", current_user.RolPerms) as contexto:
                    if contexto is False:  # No tiene permisos
                        return jsonify({'error':'El usuario actual no tiene los permisos para ejecutar esta accion.','action':'redirect'})
                events = scaizen_db.Alarma.select_alarms_given_between_date(datepicker_start, datepicker_end)
            elif type == 'eventos':
                with VerificarPermisosUsuario("DReportedeeventosPDF", current_user.RolPerms) as contexto:
                    if contexto is False:  # No tiene permisos
                        return jsonify({'error':'El usuario actual no tiene los permisos para ejecutar esta accion.','action':'redirect'})
                events = cv.EventosDistribuidor.select_given_between_date(datepicker_start, datepicker_end)
            if events:
...
# Reportes alarmas #
@app_distribuidor.route('/alarmas', methods=['GET','POST'])
@login_required
def alarmas():
    with VerificarPermisosUsuario("DReportedealarmas", current_user.RolPerms) as contexto:
        if contexto is False:  # No tiene permisos
            return redirect("/scaizen/")
    componente = 'distribuidor/alarmas'
    Descripcion = None
    Tipo_Evento = None
    if request.method == 'POST':
        with VerificarPermisosUsuario("DReportedealarmasVer", current_user.RolPerms) as contexto:
            if contexto is False:  # No tiene permisos
                return redirect("/scaizen/")
        ...
        if datepicker_start and datepicker_end and page_num:
            try:
                ...
                start_index = (page_num - 1) * elementos_por_pagina + 1
                end_index = start_index + elementos_por_pagina
                #logging.debug(cv.EventosAlarmasDistribuidor.select_events_and_alarms_between_dates(datepicker_start,datepicker_end))
                #events, total  = scaizen_db.Evento.select_events_given_between_date_for_pagination(datepicker_start, datepicker_end,page_num,elementos_por_pagina)
                alarms,total =  scaizen_db.Alarma.select_alarms_given_between_date_for_pagination(datepicker_start, datepicker_end,page_num,elementos_por_pagina)
                ...
                events_json = []
                if alarms:
                    events_json = scaizen_db.query_to_json(alarms)
                    # Registrar el evento de consulta
                    descripcion_evento = f"Consulta de alarmas desde {datepicker_start} hasta {datepicker_end}."
                    identificacion_componente_alarma = "Consulta Alarmas"
                    EventosDistribuidor.add(
                        datetime.now(), current_user.Username, 11, descripcion_evento, identificacion_componente_alarma
                    )
                    ...
                    # Registrar el evento de consulta
                    descripcion_evento = f"No se encontraron alarmas en la consulta con rango de {datepicker_start} hasta {datepicker_end}."
                    identificacion_componente_alarma = "Consulta Alarmas"
                    EventosDistribuidor.add(
                        datetime.now(), current_user.Username, 11, descripcion_evento, identificacion_componente_alarma
                    )
        ...
        return render_template('scaizen/alarmas_eventos.html',blueprint='app_distribuidor',type='alarmas', usuarios=usuarios)

# Reportes eventos #
@app_distribuidor.route('/eventos', methods=['GET','POST'])
@login_required
def eventos():
    with VerificarPermisosUsuario("DReportedeeventos", current_user.RolPerms) as contexto:
        if contexto is False:  # No tiene permisos
            return redirect("/scaizen/")
    componente = 'distribuidor/eventos'
    Descripcion = None
    Tipo_Evento = None
    if request.method == 'POST':
        ...
        if datepicker_start and datepicker_end and page_num:
            try:
                # Registrar el evento de consulta
                descripcion_evento = f"Consulta de eventos desde {datepicker_start} hasta {datepicker_end}."
                identificacion_componente_alarma = "Consulta Eventos"
                EventosDistribuidor.add(
                    datetime.now(), current_user.Username, 11, descripcion_evento, identificacion_componente_alarma
                )
                elementos_por_pagina=100
                ...
                events, total  = cv.EventosDistribuidor.select_given_between_date_for_pagination(datepicker_start, datepicker_end,page_num,elementos_por_pagina)
                ...
                if events:
                    ...
                    # Registrar el evento de consulta
                    descripcion_evento = f"No se encontraron eventos en la consulta con rango de {datepicker_start} hasta {datepicker_end}."
                    identificacion_componente_alarma = "Consulta Eventos"
                    EventosDistribuidor.add(
                        datetime.now(), current_user.Username, 11, descripcion_evento, identificacion_componente_alarma
                    )
        ...
        usuarios = [u[0] for u in usuarios]
        logging.debug(usuarios)
        return render_template('scaizen/alarmas_eventos.html',blueprint='app_distribuidor',type='eventos', usuarios=usuarios)


@app_distribuidor.route('/registrar_alarma_evento', methods=['POST', 'GET'])
@login_required
def registrar_alarma_evento():
    ...
    """
    componente = 'distribuidor/registrar'
    Descripcion = None
    Tipo_Evento = None
    ...
    try:
        if clase == 'alarma':
            ...
            nuevo_evento = cv.EventosDistribuidor.add(
                fecha=fecha_hora,
